...
日期：2026-07-11
整理人：张浩（James）
上一版：v3.0（2026-07-11）——本版修复架构评审发现的 4 个架构级漏洞、3 处内部矛盾、2 处设计倒退，并将 P1 拆分为 P1a/P1b
...
所有 Agent 间通信：扔 Redis Stream / 从 Stream 消费。没有谁直接调谁；后端 submission-task-agent 是唯一写飞书业务记录者。
...
Hermes/WorkBuddy Manifest 接入、Peer Review 实际分配、require-approval 流、Presence、Ontology Memory（Team3 OWL/Fuseki）。
...
### ADR-001 消息总线渐进替换（更新）
进程内调用（现状）→ Redis Stream（P1b）→ Hermes/OpenClaw（P3）。Envelope 格式三阶段不变，只换传输层。
