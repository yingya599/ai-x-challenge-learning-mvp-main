...
import { Bot, ChevronLeft, ChevronRight, Filter, LoaderCircle, Plus, Search, Sparkles, UserRoundCheck, X } from "lucide-react";
...
type Category = { category_id: string; title: string; job_direction: string; source_type?: string };
...
function PersonalTasksContent() {
  ...
  const [studentId, setStudentId] = useState(params.get("studentId") || "all");
  ...
  return <div className="space-y-6">
    ...
    {showCreate && canCreate && <CreateTaskModal role={role} storageReady={storageReady} interns={interns} mentors={mentors} categories={categories} onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); void load(); }} />}
  </div>;
}

function CreateTaskModal({ role, storageReady, interns, mentors, categories, onClose, onCreated }: { role: string; storageReady: boolean; interns: Intern[]; mentors: Mentor[]; categories: Category[]; onClose: () => void; onCreated: () => void }) {
  const isLeader = role === "leader";
  const [step, setStep] = useState<1 | 2>(1);
  const [form, setForm] = useState({ student_id: "", mentor_id: "", category_id: "", job_direction: "data_analysis", title: "", business_context: "", objective: "", acceptance_criteria: "", due_date: "", priority: "medium", confidentiality: "internal" });
  ...
  const organizeWithAi = async () => {
    ...
    const categoryTitle = form.category_id.startsWith("custom-") ? undefined : availableCategories.find((item) => item.category_id === form.category_id)?.title;
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4"><form onSubmit={submit} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
    <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold text-gray-900">发布个人任务</h2><p className="mt-1 text-sm text-gray-500">第 {step} 步，共 2 步 · {step === 1 ? "选择执行人和可选模板" : "填写真实业务任务要求"}</p></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><X className="h-5 w-5" /></button></div>
    <div className="mt-5 grid grid-cols-2 gap-3"><div className={`rounded-lg border px-3 py-2 text-sm ${step === 1 ? "border-primary-300 bg-primary-50 font-medium text-primary-800" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>1. 分配对象与模板</div><div className={`rounded-lg border px-3 py-2 text-sm ${step === 2 ? "border-primary-300 bg-primary-50 font-medium text-primary-800" : "border-gray-200 text-gray-400"}`}>2. 任务内容与期限</div></div>
    ...
    {step === 1 && <section className="mt-5 rounded-xl border border-primary-200 bg-primary-50/50 p-4"><div className="flex items-center gap-2 font-medium text-primary-900"><UserRoundCheck className="h-5 w-5" />先确定任务分给谁</div><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="让哪名实习生执行？ *"><select value={form.student_id} onChange={(event) => { const intern = interns.find((item) => item.student_id === event.target.value); setForm((current) => ({ ...current, student_id: event.target.value, mentor_id: intern?.mentor_id || "", job_direction: intern?.job_direction || current.job_direction, category_id: intern ? `custom-${intern.job_direction}` : "", title: "" })); }} className="input"><option value="">请选择实习生</option>{interns.map((item) => <option key={item.student_id} value={item.student_id}>{item.name} · {item.position || directionLabels[item.job_direction]}</option>)}</select></Field>{isLeader ? <Field label="负责带教 *"><select value={form.mentor_id} onChange={(event) => update("mentor_id", event.target.value)} className="input"><option value="">请选择带教</option>{mentors.map((item) => <option key={item.mentorId} value={item.mentorId}>{item.name}</option>)}</select></Field> : <div className="rounded-lg bg-white p-3 text-sm text-gray-600"><p className="text-xs text-gray-400">负责带教</p><p className="mt-1 font-medium text-gray-800">当前带教账号（自动绑定）</p></div>}</div>{selectedIntern && <div className="mt-4"><p className="mb-2 text-xs text-primary-800">{selectedIntern.name} · {selectedIntern.department || "未填写部门"} · {directionLabels[selectedIntern.job_direction] || selectedIntern.position}</p><Field label={`${directionLabels[selectedIntern.job_direction] || "该岗位"}快捷模板（可选）`}><select value={form.category_id} onChange={(event) => { const category = availableCategories.find((item) => item.category_id === event.target.value); setForm((current) => ({ ...current, category_id: event.target.value, job_direction: category?.job_direction || current.job_direction, title: category?.title || "" })); }} className="input"><option value={`custom-${selectedIntern.job_direction}`}>其他 / 自定义任务（不套用模板）</option>{availableCategories.map((item) => <option key={item.category_id} value={item.category_id}>{item.title}</option>)}</select></Field><p className="mt-2 text-xs leading-5 text-gray-500">这 6 类只是该岗位常见任务的快捷起点，不限制真实业务；选择“其他 / 自定义”后可直接用自然语言描述任何任务。</p></div>}</section>}
    {step === 2 && <>
      <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700"><span className="font-medium">{selectedIntern?.name}</span> · {directionLabels[form.job_direction]} · {availableCategories.find((item) => item.category_id === form.category_id)?.title || "其他 / 自定义任务"}</div>
      <section className="mt-5 rounded-2xl border border-primary-200 bg-gradient-to-br from-primary-50 to-white p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div><div className="flex items-center gap-2 font-semibold text-primary-900"><Bot className="h-5 w-5" />让 AI 帮你整理任务</div><p className="mt-1 text-xs leading-5 text-gray-600">直接描述任务即可，不必按固定模板，也不要求来自微信。AI 只生成草稿，发布前仍由带教确认。</p></div>
